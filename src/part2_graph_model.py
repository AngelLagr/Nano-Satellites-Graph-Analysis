...
output_dir = os.path.join(base_dir, "outputs", "part2")  # Chemin vers le dossier de sortie pour la partie 2
...
for density, file_path in file_paths.items():
    ...
    for max_range in ranges:
        ...
        case_dir = os.path.join(output_dir, f"{density}_{max_range}km")
        ...
        with open(analysis_file_path, "w") as f:
            ...
            f.write(f"Degré moyen ({density}, {max_range} km): {degree_mean}\n\n")
            ...
            plt.title(f'Distribution du degré ({density}, {max_range} km)')
            plt.xlabel('Degré')
            ...
            f.write(f"Moyenne du degré de clustering ({density}, {max_range} km): {round(clustering_mean,3)}\n\n")
            
            clustering = list(nx.clustering(G).values())
            clustering_hist_file = os.path.join(case_dir, "clustering_distribution.png")
            plt.hist(clustering, bins=20, alpha=0.75)
            plt.title(f'Distribution du degré de clustering ({density}, {max_range} km)')
            ...
            f.write(f"Nombre de cliques ({density}, {max_range} km): {len(cliques)}\n\n")
            cliques_hist_file = os.path.join(case_dir, "cliques_distribution.png")
            plt.hist(cliques_sizes, bins=20, alpha=0.75)
            plt.title(f'Distribution des de cliques ({density}, {max_range} km)')
            ...
            f.write(f"Nombre de composantes connexes ({density}, {max_range} km): {len(components)}\n\n")
            ...
            for source in G.nodes():
                ...
                all_shortest_paths[source] = dict(nx.shortest_path_length(G, source=source))
            ...
            if shortest_paths_lengths:
                ...
                f.write(f"Longueur moyenne des plus courts chemins (sans poids sur les arrêtes) ({density}, {max_range} km): {avg_shortest_path_length}\n\n")
            ...
            plt.title(f'Distribution des plus courts chemins (sans poids sur les arrêtes) ({density}, {max_range} km)')
            ...
            f.write(f"Nombre de plus courts chemins (sans poids sur les arrêtes) ({density}, {max_range} km): {num_shortest_paths}\n\n")

        print(f"Analyse pour {density}, {max_range} km sauvegardée dans: {case_dir}")
